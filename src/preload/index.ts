...
const api = {
	app: {
		...
		onCloseRequested: (callback: () => void): (() => void) => {
			...
			return () =>
				typedIpcRenderer.removeListener("app:close-requested", listener);
		},
	},
	dependency: {
		getStatus: (): Promise<DependencyStatus> =>
			typedIpcRenderer.invoke("dependency:get-status"),
	} satisfies DependencyAPIRenderer,
	adb: {
		listDevices: (): Promise<DeviceInfo[]> =>
			typedIpcRenderer.invoke("adb:list-devices"),
		...
		startTrackingDevices: (): void =>
			typedIpcRenderer.send("adb:start-tracking-devices"),
		stopTrackingDevices: (): void =>
			typedIpcRenderer.send("adb:stop-tracking-devices"),
		onDeviceAdded: (callback: (device: DeviceInfo) => void): (() => void) => {
			const listener = (_: IpcRendererEvent, device: DeviceInfo): void =>
				callback(device);
			typedIpcRenderer.on("adb:device-added", listener);
			return () =>
				typedIpcRenderer.removeListener("adb:device-added", listener);
		},
		onDeviceRemoved: (callback: (device: DeviceInfo) => void): (() => void) => {
			...
			return () =>
				typedIpcRenderer.removeListener("adb:device-removed", listener);
		},
		onDeviceChanged: (callback: (device: DeviceInfo) => void): (() => void) => {
			...
			return () =>
				typedIpcRenderer.removeListener("adb:device-changed", listener);
		},
		onTrackerError: (callback: (error: string) => void): (() => void) => {
			const listener = (_: IpcRendererEvent, error: string): void =>
				callback(error);
			typedIpcRenderer.on("adb:device-tracker-error", listener);
			return () =>
				typedIpcRenderer.removeListener("adb:device-tracker-error", listener);
		},
		onInstallationCompleted: (
			callback: (deviceId: string) => void,
		): (() => void) => {
			const listener = (_event: IpcRendererEvent, deviceId: string): void =>
				callback(deviceId);
			typedIpcRenderer.on("adb:installation-completed", listener);
			return () =>
				typedIpcRenderer.removeListener("adb:installation-completed", listener);
		},
		getApplicationLabel: (
			serial: string,
			packageName: string,
	...
	games: {
		getGames: (): Promise<GameInfo[]> =>
			typedIpcRenderer.invoke("games:get-games"),
		getBlacklistGames: () =>
		...
		forceSync: (): Promise<GameInfo[]> =>
			typedIpcRenderer.invoke("games:force-sync-games"),
		getTrailerUrl: (
			gameName: string,
		...
		onDownloadProgress: (
			...
			return () =>
				typedIpcRenderer.removeListener("games:download-progress", listener);
		},
		onBackgroundSyncComplete: (
			callback: (games: GameInfo[]) => void,
		): (() => void) => {
			const listener = (_: IpcRendererEvent, games: GameInfo[]): void =>
				callback(games);
			typedIpcRenderer.on("games:background-sync-complete", listener);
			return () =>
				typedIpcRenderer.removeListener(
					"games:background-sync-complete",
					listener,
				);
		},
		onBackgroundSyncError: (
	...
	downloads: {
		getQueue: (): Promise<DownloadItem[]> =>
			typedIpcRenderer.invoke("download:get-queue"),
		...
		copyObbFolder: (folderPath: string, deviceId: string): Promise<boolean> =>
			typedIpcRenderer.invoke(
				"downloads:copy-obb-folder",
				folderPath,
				deviceId,
			),
		onQueueUpdated: (
			callback: (queue: DownloadItem[]) => void,
		): (() => void) => {
			const listener = (_: IpcRendererEvent, queue: DownloadItem[]): void =>
				callback(queue);
			typedIpcRenderer.on("download:queue-updated", listener);
			return () =>
				typedIpcRenderer.removeListener("download:queue-updated", listener);
		},
		...
		setAppConnectionState: (
			...
			ipcRenderer.send(
				"download:set-app-connection-state",
				selectedDevice,
				isConnected,
			),
	...
	uploads: {
		...
		getQueue: (): Promise<UploadItem[]> =>
			typedIpcRenderer.invoke("upload:get-queue"),
		addToQueue: (
			packageName: string,
			gameName: string,
			versionCode: number,
		...
		onUploadProgress: (
			...
			const listener = (
				_: IpcRendererEvent,
				progress: UploadPreparationProgress,
			): void => callback(progress);
		...
		onQueueUpdated: (callback: (queue: UploadItem[]) => void): (() => void) => {
			const listener = (_: IpcRendererEvent, queue: UploadItem[]): void =>
				callback(queue);
			typedIpcRenderer.on("upload:queue-updated", listener);
			return () =>
				typedIpcRenderer.removeListener("upload:queue-updated", listener);
		},
	} satisfies UploadAPIRenderer,
	// Update APIs
	updates: {
		checkForUpdates: (): Promise<void> =>
			typedIpcRenderer.invoke("update:check-for-updates"),
		openDownloadPage: (url: string): void =>
			typedIpcRenderer.send("update:download", url),
		openReleasesPage: (): void => typedIpcRenderer.send("update:open-releases"),
		openRepositoryPage: (): void =>
			typedIpcRenderer.send("update:open-repository"),
		...
		onCheckingForUpdate: (callback: () => void): (() => void) => {
			...
			return () =>
				typedIpcRenderer.removeListener("update:checking-for-update", listener);
		},
		onUpdateAvailable: (callback: (info: UpdateInfo) => void): (() => void) => {
			const listener = (_: IpcRendererEvent, info: UpdateInfo): void =>
				callback(info);
			typedIpcRenderer.on("update:update-available", listener);
			return () =>
				typedIpcRenderer.removeListener("update:update-available", listener);
		},
		onUpdateError: (callback: (error: Error) => void): (() => void) => {
			const listener = (_: IpcRendererEvent, error: Error): void =>
				callback(error);
		...
		onDownloadProgress: (
			...
			): void => callback(progressInfo);
			typedIpcRenderer.on("update:download-progress", listener);
			return () =>
				typedIpcRenderer.removeListener("update:download-progress", listener);
		},
		onUpdateDownloaded: (
			callback: (updateInfo: UpdateInfo) => void,
		): (() => void) => {
			const listener = (_: IpcRendererEvent, updateInfo: UpdateInfo): void =>
				callback(updateInfo);
			typedIpcRenderer.on("update:update-downloaded", listener);
			return () =>
				typedIpcRenderer.removeListener("update:update-downloaded", listener);
		},
	} satisfies UpdateAPIRenderer,
	settings: {
		getDownloadPath: (): Promise<string> =>
			typedIpcRenderer.invoke("settings:get-download-path"),
		...
		getLanguage: (): Promise<AppLanguage> =>
			typedIpcRenderer.invoke("settings:get-language"),
	...
	logs: {
		...
		openLogFolder: (): Promise<void> =>
			typedIpcRenderer.invoke("logs:open-log-folder"),
		openLogFile: (): Promise<void> =>
			typedIpcRenderer.invoke("logs:open-log-file"),
	...
	backup: {
		...
		deleteBackup: (backupId) =>
			typedIpcRenderer.invoke("backup:delete", backupId),
		setVerification: (backupId, result) =>
			typedIpcRenderer.invoke("backup:set-verification", backupId, result),
		reportFailure: (backupId) =>
			typedIpcRenderer.invoke("backup:report-failure", backupId),
		getProfile: (packageName) =>
			typedIpcRenderer.invoke("backup:get-profile", packageName),
	...
	mirrors: {
		...
		removeMirror: (id: string) =>
			typedIpcRenderer.invoke("mirrors:remove-mirror", id),
		setActiveMirror: (id: string) =>
			typedIpcRenderer.invoke("mirrors:set-active-mirror", id),
		clearActiveMirror: () =>
			typedIpcRenderer.invoke("mirrors:clear-active-mirror"),
		testMirror: (id: string) =>
			typedIpcRenderer.invoke("mirrors:test-mirror", id),
		...
		onMirrorTestProgress: (
			...
			return () =>
				typedIpcRenderer.removeListener("mirrors:test-progress", listener);
		},
		onMirrorsUpdated: (callback: (mirrors: Mirror[]) => void): (() => void) => {
			const listener = (_: IpcRendererEvent, mirrors: Mirror[]): void =>
				callback(mirrors);
			typedIpcRenderer.on("mirrors:mirrors-updated", listener);
			return () =>
				typedIpcRenderer.removeListener("mirrors:mirrors-updated", listener);
		},
	...
	dialog: {
		...
		showFilePicker: (options?: {
			...
			typedIpcRenderer.invoke("dialog:show-file-picker", options),
	...
	wifiBookmarks: {
		getAll: (): Promise<WiFiBookmark[]> =>
			typedIpcRenderer.invoke("wifi-bookmarks:get-all"),
		...
		remove: (id: string): Promise<boolean> =>
			typedIpcRenderer.invoke("wifi-bookmarks:remove", id),
	...
	onDependencyProgress: (
		...
		return () =>
			typedIpcRenderer.removeListener("dependency-progress", listener);
	},
	onDependencySetupComplete: (
		callback: (status: DependencyStatus) => void,
	): (() => void) => {
		const listener = (_: IpcRendererEvent, status: DependencyStatus): void =>
			callback(status);
		typedIpcRenderer.on("dependency-setup-complete", listener);
		return () =>
			typedIpcRenderer.removeListener("dependency-setup-complete", listener);
	},
	onDependencySetupError: (
		...
		return () =>
			typedIpcRenderer.removeListener("dependency-setup-error", listener);
	},
};
