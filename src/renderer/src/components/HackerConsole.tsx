...
const HackerConsole: React.FC = () => {
  ...
  const line = (label: string, value: string, ok?: boolean): React.JSX.Element => (
    <div style={{ display: 'flex', gap: '6px', alignItems: 'baseline' }}>
      ...
      <span
        style={{
          color: ok === false ? '#ff6666' : ok === true ? 'var(--vrcd-neon)' : 'var(--vrcd-neon)',
          overflow: 'hidden',
          textOverflow: 'ellipsis',
          whiteSpace: 'nowrap',
          fontWeight: 700,
          textShadow: '0 0 6px rgba(var(--vrcd-neon-raw),0.4)'
        }}
      >
        {value}
      </span>
  ...
  return (
    ...
    >
      ...
      {line(
        'DEV',
        isConnected && deviceName ? deviceName.toUpperCase().slice(0, 16) : 'OFFLINE',
        isConnected
      )}
