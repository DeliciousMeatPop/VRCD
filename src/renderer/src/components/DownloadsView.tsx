...
const DownloadsView: React.FC<DownloadsViewProps> = ({ onClose }) => {
	...
	const handleInstallFromCompleted = (releaseName: string): void => {
		if (!releaseName || !selectedDevice) {
			console.error(
				"Missing releaseName or selectedDevice for install from completed action",
			);
			window.alert("Cannot start installation: Missing required information.");
			return;
		}
		console.log(
			`Requesting install from completed for ${releaseName} on ${selectedDevice}`,
		);
		window.api.downloads
			.installFromCompleted(releaseName, selectedDevice)
			.catch((err) => {
	...
	const performUninstall = async (game: GameInfo): Promise<void> => {
		if (!game || !game.packageName || !selectedDevice) {
			console.error(
				"Cannot uninstall: Missing game data, package name, or selected device",
			);
	...
	const isInstalled = (releaseName: string): boolean => {
		return games.some(
			(game) => game.releaseName === releaseName && game.isInstalled,
		);
	};
	...
	return (
		<div className={styles.root}>
					...
					{queue.map((item) => (
						<div key={item.releaseName} className={styles.itemRow}>
							...
							<div
								...
								onClick={() => {
									let gameToOpen = games.find(
										(g) => g.releaseName === item.releaseName,
									);
									if (!gameToOpen) {
										console.log(
											"Game not found by release name, trying by package name",
										);
										gameToOpen = games.find(
											(g) => g.packageName === item.packageName,
										);
									}
									if (gameToOpen) {
							...
							<div className={styles.progressStatus}>
								...
								{item.status === "Queued" && (
									<Text className={styles.statusText}>Queued</Text>
								)}
								...
								{item.status === "Completed" &&
									...
									!sideloadingDisabled && (
										<Button
											...
											appearance="primary"
											onClick={() =>
												handleInstallFromCompleted(item.releaseName)
											}
								...
								{item.status === "Queued" && (
									<>
										<Button
											...
											size="small"
											appearance="subtle"
											onClick={() => moveQueuedUp(item.releaseName)}
										...
										<Button
											...
											size="small"
											appearance="subtle"
											onClick={() => moveQueuedDown(item.releaseName)}
			...
			<ErrorDetailDialog
				...
				onRetry={
					errorDetail ? () => retryDownload(errorDetail.releaseName) : undefined
				}
			/>
