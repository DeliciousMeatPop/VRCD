...
const TransferStrip: React.FC = () => {
	...
	const entries = useMemo<Entry[]>(() => {
		const downloads: Entry[] = downloadQueue
			.filter((d) =>
				["Queued", "Downloading", "Extracting", "Installing"].includes(
					d.status,
				),
			)
			.map((d) => ({
	...
	if (entries.length === 0) {
		return (
			...
			>
				...
				<span style={{ color: "rgba(var(--vrcd-neon-raw),0.3)" }}>
	...
	const progressPct =
		e.progress != null ? Math.max(0, Math.min(100, e.progress)) : null;
	...
	return (
		...
		>
			...
			{e.speed && (
				<span
					style={{ color: "rgba(var(--vrcd-neon-raw),0.75)", flexShrink: 0 }}
				>
					{e.speed}
				</span>
			)}
			{e.eta && (
				<span
					style={{ color: "rgba(var(--vrcd-neon-raw),0.5)", flexShrink: 0 }}
				>
					ETA {e.eta}
				</span>
			)}
