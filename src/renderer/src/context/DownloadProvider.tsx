...
export const DownloadProvider: React.FC<DownloadProviderProps> = ({
	...
	useEffect(() => {
		...
		const removeUpdateListener = window.api.downloads.onQueueUpdated(
			(updatedQueue) => {
				...
				for (const item of updatedQueue) {
					if (
						...
						!prevCompleted.has(item.releaseName)
					) {
	...
	const settleResolveExisting = useCallback(
		async (action: "reinstall" | "redownload") => {
			...
			try {
				...
				const result = await window.api.downloads.addToQueueResolveExisting(
					prompt.game,
					action,
				);
	...
	const removeFromQueue = useCallback(
		async (releaseName: string): Promise<void> => {
			try {
	...
	const removeFromQueueOnly = useCallback(
		async (releaseName: string): Promise<void> => {
			try {
				await window.api.downloads.removeFromQueueOnly(releaseName);
			} catch (err) {
				console.error(
					"Error removing game from download queue (keep files) via IPC:",
					err,
				);
				setError("Failed to remove item from queue.");
			}
	...
	const moveToFront = useCallback(
		async (releaseName: string): Promise<boolean> => {
			try {
	...
	const moveQueuedDown = useCallback(
		async (releaseName: string): Promise<boolean> => {
			try {
	...
	const deleteFiles = useCallback(
		async (releaseName: string): Promise<boolean> => {
			try {
				const success =
					await window.api.downloads.deleteDownloadedFiles(releaseName);
