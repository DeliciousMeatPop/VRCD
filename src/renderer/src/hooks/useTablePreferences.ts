...
function load(): TablePreferences {
	...
	try {
		...
		if (raw) {
			...
			if (parsed.cardSortDir !== "asc" && parsed.cardSortDir !== "desc")
				parsed.cardSortDir = "asc";
	...
	const setPrefs = useCallback((update: Partial<TablePreferences>) => {
		setState((prev) => {
			...
			if (next.viewMode !== "table" && next.viewMode !== "cards")
				next.viewMode = "table";
			if (next.cardSortDir !== "asc" && next.cardSortDir !== "desc")
				next.cardSortDir = "asc";
			if (next.tableSortDir !== "asc" && next.tableSortDir !== "desc")
				next.tableSortDir = "asc";
			try {
