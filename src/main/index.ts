...
function createWindow(): void {
	...
	const { height: workH, width: workW } =
	...
	mainWindow.on("ready-to-show", async () => {
		if (mainWindow) {
			...
			try {
				const initialized = await dependencyService.initialize(
					sendDependencyProgress,
				);
				if (initialized === "INITIALIZING") {
					console.log(
						"DependencyService already initializing, skipping duplicate request.",
					);
					return;
				}
				...
				if (mainWindow && !mainWindow.isDestroyed()) {
					...
					try {
						console.log(
							"Dependencies ready, initializing dependent services...",
						);
						...
						console.log(
							`Game Service initialization status: ${gameServiceStatus}`,
						);
						...
						console.error(
							"Error initializing dependent services:",
							serviceInitError,
						);
					...
					typedWebContentsSend.send(mainWindow, "dependency-setup-error", {
						message:
							error instanceof Error
								? error.message
								: "Unknown dependency initialization error",
...
app.whenReady().then(async () => {
	...
	typedIpcMain.handle("app:reset-app-data", async () => {
		...
		try {
			await fsPromises.writeFile(
				join(app.getPath("userData"), "pending-data-reset"),
				"",
			);
		} catch (e) {
			return {
	...
	// --- Dependency Handlers ---
	typedIpcMain.handle("dependency:get-status", async () =>
		dependencyService.getStatus(),
	);
	...
	typedIpcMain.handle(
		...
		async (_event, serial, packageName) => {
			console.log(
				`IPC adb:uninstallPackage called for ${packageName} on ${serial}`,
			);
	...
	typedIpcMain.handle("adb:deleteGameFiles", async (_event, releaseName) => {
		...
		try {
			...
			console.error(
				`[IPC] adb:deleteGameFiles - Error deleting ${gamePath}:`,
				error,
			);
	...
	typedIpcMain.on("adb:start-tracking-devices", () => {
		if (mainWindow) adbService.startTrackingDevices(mainWindow);
			...
			console.error(
				"Cannot start tracking devices, mainWindow is not available.",
			);
	});
	typedIpcMain.on("adb:stop-tracking-devices", () =>
		adbService.stopTrackingDevices(),
	);
	typedIpcMain.handle(
		"adb:get-application-label",
		async (_event, serial, packageName) => {
	...
	typedIpcMain.handle("games:get-blacklist-games", async () =>
		gameService.getBlacklistGames(),
	);
	typedIpcMain.handle(
		"games:add-to-blacklist",
		async (_event, packageName, version) => {
			return gameService.addToBlacklist(packageName, version);
		},
	);
	typedIpcMain.handle(
		"games:remove-from-blacklist",
		async (_event, packageName) => {
	...
	typedIpcMain.handle("games:get-last-sync-time", async () =>
		gameService.getLastSyncTime(),
	);
	...
	typedIpcMain.handle(
		"games:get-trailer-url",
		async (_event, gameName, packageName) => {
	...
	typedIpcMain.handle("download:add", (_event, game) =>
		downloadService.addToQueue(game),
	);
	...
	typedIpcMain.handle(
		...
		(_event, releaseName, deviceId) => {
			...
			downloadService
				.installFromCompleted(releaseName, deviceId)
				.catch((err) => {
	...
	typedIpcMain.handle(
		...
		async (_event, packageName, gameName, versionCode, deviceId) => {
			...
			try {
				...
				console.error(
					`[IPC Handler Error] Upload preparation failed for ${packageName}:`,
					err,
				);
				return null;
			}
	...
	typedIpcMain.handle(
		...
		async (_event, packageName, gameName, versionCode, deviceId) => {
			...
			return uploadService.addToQueue(
				packageName,
				gameName,
				versionCode,
				deviceId,
	...
	typedIpcMain.handle("upload:add-local-items", async (_event, paths) => {
		console.log(
			`[IPC] Adding local items to upload queue: ${paths.join(", ")}`,
		);
	...
	typedIpcMain.handle("download:remove-only", async (_event, releaseName) => {
		console.log(
			`[IPC] Removing from download queue (keep files): ${releaseName}`,
		);
	...
	ipcMain.on(
		...
		(_event, selectedDevice, isConnected) => {
			console.log(
	...
	// --- Settings Handlers ---
	typedIpcMain.handle("settings:get-download-path", () =>
	...
	typedIpcMain.handle("settings:get-color-scheme", () =>
		settingsService.getColorScheme(),
	);
	typedIpcMain.handle("settings:set-color-scheme", (_event, scheme) =>
		settingsService.setColorScheme(scheme),
	);

	typedIpcMain.handle("settings:get-server-config", () =>
		settingsService.getServerConfig(),
	);
	...
	typedIpcMain.handle("settings:get-language", () =>
		settingsService.getLanguage(),
	);
	typedIpcMain.handle("settings:set-language", (_event, lang) =>
	...
	typedIpcMain.handle(
		"wifi-bookmarks:add",
		async (_event, name, ipAddress, port) => {
	...
	typedIpcMain.handle(
		"wifi-bookmarks:update-last-connected",
		async (_event, id) => {
	...
	typedIpcMain.handle(
		"downloads:install-manual",
		async (_event, filePath, deviceId) => {
			console.log(
				`[IPC] Manual install requested for ${filePath} on device ${deviceId}`,
			);
	...
	typedIpcMain.handle(
		...
		async (_event, folderPath, deviceId) => {
			console.log(
				`[IPC] OBB folder copy requested for ${folderPath} on device ${deviceId}`,
			);
	...
	typedIpcMain.handle("manual:classify-path", async (_event, targetPath) => {
		...
		try {
			...
			if (stat.isDirectory()) {
				...
				const hasObb = entries.some(
					(e) => e.isFile() && e.name.toLowerCase().endsWith(".obb"),
				);
				const looksLikePackage = /^[a-z0-9_]+(\.[a-z0-9_]+)+$/i.test(name);
				if (hasObb || looksLikePackage)
					return { kind: "obbFolder" as const, name };
