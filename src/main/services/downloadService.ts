...
class DownloadService extends EventEmitter implements DownloadAPI {
	...
	private getTargetDeviceForInstallation(): string | null {
		...
		if (!this.appIsConnected || !this.appSelectedDevice) {
			console.log(
				"[Service] App is not connected to any device, skipping installation",
			);
			return null;
		}
	...
	public addToQueue(game: GameInfo): Promise<AddToQueueResult> {
		...
		if (!game.releaseName) {
			console.error(
				`Cannot add game ${game.name} to queue: Missing releaseName.`,
			);
			return Promise.resolve("duplicate");
		}
	...
	public async addToQueueResolveExisting(
		...
		try {
			...
			console.error(
				`[Service] Failed to wipe ${folderPath} before redownload:`,
				err,
			);
		}
	...
	private importExistingAsCompleted(game: GameInfo): void {
		...
		console.log(
			`Imported existing folder for ${game.releaseName} as Completed.`,
		);
		this.emitUpdate();
	}
	...
	private async addToQueueInternal(
		...
		if (existing) {
			...
			console.log(
				`Re-adding game ${game.releaseName} after previous ${existing.status}.`,
			);
	...
	public async removeFromQueue(releaseName: string): Promise<void> {
		...
		if (removed) {
			console.log(
				`[Service] Removed ${releaseName} from queue (status: ${item.status}).`,
			);
			this.emitUpdate();
		}
	}
		...
		while (this.activeCount < maxConcurrent) {
			...
			if (!nextItem) {
				if (this.activeCount === 0) {
					console.log(
						"[Service ProcessQueue] No queued items and no active operations",
					);
				}
				return;
			}
	...
	private async runPipeline(nextItem: DownloadItem): Promise<void> {
		...
		try {
			...
			const extractionSuccess =
				await this.extractionProcessor.startExtraction(itemAfterDownload);
			...
			const itemAfterExtraction = this.queueManager.findItem(
				itemAfterDownload.releaseName,
			);
			...
			const installationSuccess =
				await this.installationProcessor.startInstallation(
					itemAfterExtraction,
					finalTargetDeviceId,
				);
			const installDuration = ((Date.now() - installStartTime) / 1000).toFixed(
				1,
			);
	...
	private async runResumePipeline(nextItem: DownloadItem): Promise<void> {
		...
		try {
			const downloadResult =
				await this.downloadProcessor.resumeDownload(nextItem);
			if (!downloadResult.success) {
				console.log(
	...
	private updateItemStatus(
		...
		if (updated) {
			...
			console.warn(
				`[Service updateItemStatus] Failed update for non-existent item: ${releaseName}`,
			);
		}
	}
	...
	public cancelUserRequest(releaseName: string): Promise<void> {
		...
		if (!item) {
			console.warn(
				`[Service cancelUserRequest] Cannot cancel ${releaseName} - not found.`,
			);
	...
	public async retryDownload(releaseName: string): Promise<void> {
		...
		if (item && item.status === "InstallError") {
			...
			if (
				...
				(await this.folderContainsApk(folderPath))
			) {
				console.log(
				...
				if (deviceId && !this.sideloadingDisabled) {
					try {
						...
						console.error(
							`[Service] Retry re-install failed for ${releaseName}:`,
							err,
						);
					}
				} else {
		...
		if (
			...
			if (this.downloadProcessor.isDownloadActive(releaseName)) {
				...
				this.downloadProcessor.cancelDownload(
					releaseName,
					"Error",
			...
			if (updated) {
				...
				console.warn(
					`[Service Retry] Failed to update ${releaseName} for retry.`,
				);
			}
		} else {
			console.warn(
				`[Service Retry] Cannot retry ${releaseName} - status: ${item?.status}`,
			);
		}
		return Promise.resolve();
	}
	...
	public async deleteDownloadedFiles(releaseName: string): Promise<boolean> {
		...
		if (!existsSync(downloadPath)) {
			console.log(
				`Path not found for ${releaseName}: ${downloadPath}. Removing item.`,
			);
			const removed = this.queueManager.removeItem(releaseName);
			if (removed) this.emitUpdate();
			return Promise.resolve(true);
		...
		try {
			...
			console.error(
				`Error deleting ${downloadPath} for ${releaseName}:`,
				error,
			);
		...
		try {
			...
			for (const entry of entries) {
				...
				try {
					...
					if (
						inner.some(
							(e) => e.isFile() && e.name.toLowerCase().endsWith(".apk"),
						)
					)
						return true;
				} catch {
		...
		for (const { tool, args, parse } of candidates) {
			try {
				const stdout = await new Promise<string>((resolve, reject) => {
					execFile(tool, args, { timeout: 10000 }, (err, out) =>
	...
	private async inferPackageNameFromFolder(
		folderPath: string,
	): Promise<string> {
		try {
			const contents = await fs.readdir(folderPath);
			if (!contents.some((f) => f.toLowerCase().endsWith(".apk"))) return "";
			const potentials = contents.filter(
				(f) => f.includes(".") && !f.includes(" ") && f.length > 5,
			);
	...
	public async scanDownloadFolder(): Promise<{
		...
		try {
			...
			for (const g of games) {
				if (g.releaseName && g.packageName)
					catalogPackages.set(g.releaseName, g.packageName);
			}
		} catch {
		...
		for (const dirName of subdirs) {
			...
			if (!existing) {
				...
				const packageName = await this.resolvePackageName(
					dirName,
					folderPath,
					catalogPackages,
				);
				...
				if (!existing.packageName) {
					const pkg = await this.resolvePackageName(
						dirName,
						folderPath,
						catalogPackages,
					);
	...
	public async installFromCompleted(
		releaseName: string,
		deviceId: string,
	): Promise<void> {
		console.log(
			`[Service] Request to install completed item: ${releaseName} on ${deviceId}`,
		);
		const item = this.queueManager.findItem(releaseName);

		if (!item) {
			console.error(
				`[Service installFromCompleted] Item not found: ${releaseName}`,
			);
		...
		if (targetDeviceForInstall !== deviceId) {
			...
			throw new Error(
				`App is connected to a different device (${targetDeviceForInstall}).`,
			);
		}
		...
		try {
			...
			if (!targetDevice) {
				...
				throw new Error(
					`Target device ${deviceId} not found or not authorized.`,
				);
			}
		} catch (err) {
			console.error(
	...
	private async installSingleManualFolder(
		...
		try {
			...
			if (apkPath) {
				...
				if (!packageName)
					packageName = await this.inferPackageNameFromFolder(folderPath);
			}
		} catch (error) {
			console.log(
				`[Service installManualFile] Could not analyze folder structure: ${error}`,
			);
		}
		...
		if (success) {
			console.log(
				`[Service installManualFile] Successfully installed folder: ${folderPath}`,
			);
			this.emit("installation:success", deviceId);
		}
		return success;
	...
	public async installManualFile(
		filePath: string,
		deviceId: string,
	): Promise<boolean> {
		console.log(
			`[Service] Manual install requested for ${filePath} on device ${deviceId}`,
		);
		...
		try {
			...
			console.error(
				`[Service installManualFile] Error verifying target device ${deviceId}:`,
				err,
			);
			return false;
		}
		...
		if (!existsSync(filePath)) {
			console.error(
				`[Service installManualFile] File/folder not found: ${filePath}`,
			);
			return false;
		}
		...
		try {
			...
			if (stats.isFile() && filePath.toLowerCase().endsWith(".apk")) {
				...
				console.log(
					`[Service installManualFile] Installing single APK: ${filePath}`,
				);
				const success = await this.adbService.installPackage(
					deviceId,
					filePath,
					{
						flags: ["-r", "-g"],
				...
				if (success) {
					console.log(
						`[Service installManualFile] Successfully installed APK: ${filePath}`,
					);
					this.emit("installation:success", deviceId);
				}
				return success;
				...
				console.log(
					`[Service installManualFile] Inspecting folder: ${filePath}`,
				);
				...
				const isGameFolder = async (dir: string): Promise<boolean> => {
					try {
						...
						return entries.some(
							(e) =>
								e.isFile() &&
								(e.name.toLowerCase().endsWith(".apk") ||
									e.name.toLowerCase() === "install.txt"),
						);
					} catch {
						return false;
					}
				...
				console.log(
					`[Service installManualFile] Installing from ZIP: ${filePath}`,
				);

				const sevenZipPath = dependencyService.get7zPath();
				if (!sevenZipPath) {
					console.error(
						"[Service installManualFile] 7zip not found, cannot extract ZIP",
					);
					return false;
				}
				...
				try {
					...
					if (success) {
						console.log(
							`[Service installManualFile] Successfully installed from ZIP: ${filePath}`,
						);
						this.emit("installation:success", deviceId);
					}
					return success;
				...
				console.error(
					`[Service installManualFile] Unsupported file type: ${filePath}`,
				);
				return false;
			}
		} catch (error) {
	...
	public async copyObbFolder(
		folderPath: string,
		deviceId: string,
	): Promise<boolean> {
		console.log(
			`[Service] OBB folder copy requested for ${folderPath} on device ${deviceId}`,
		);
		...
		try {
			...
			console.error(
				`[Service copyObbFolder] Error verifying target device ${deviceId}:`,
				err,
			);
			return false;
		}
		...
		try {
			...
			if (!stats.isDirectory()) {
				console.error(
					`[Service copyObbFolder] Path is not a directory: ${folderPath}`,
				);
				return false;
			}
			...
			console.log(
				`[Service copyObbFolder] Creating OBB base directory: ${obbBasePath}`,
			);
			try {
				await this.adbService.runShellCommand(
					deviceId,
					`mkdir -p "${obbBasePath}"`,
				);
			} catch (mkdirError) {
			...
			console.log(
				`[Service copyObbFolder] Copying folder ${folderPath} to ${targetObbPath}`,
			);
			const success = await this.adbService.pushFileOrFolder(
				deviceId,
				folderPath,
				targetObbPath,
			);
			...
			if (success) {
				console.log(
					`[Service copyObbFolder] Successfully copied OBB folder to ${targetObbPath}`,
				);
			} else {
				console.error(
					`[Service copyObbFolder] Failed to copy OBB folder to ${targetObbPath}`,
				);
			}
			...
			console.error(
				`[Service copyObbFolder] Error during OBB folder copy of ${folderPath}:`,
				error,
			);
			return false;
		}
