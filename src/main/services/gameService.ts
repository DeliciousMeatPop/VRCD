...
class GameService extends EventEmitter implements GamesAPI {
	...
	async initialize(force?: boolean): Promise<ServiceStatus> {
		...
		try {
			...
			if (syncNeeded) {
				console.log(
					"[GameService] Stale or missing meta data - starting background sync.",
				);
	...
	private async loadConfig(): Promise<void> {
		try {
			...
			if (fromSettings.baseUri && fromSettings.password) {
				...
				console.log(
					"Loaded server config from settings - baseUri:",
			...
			if (diskConfig && diskConfig.baseUri && diskConfig.password) {
				...
				console.log(
					"Loaded server config from vrp-config.json and migrated to settings",
				);
				return;
			}
			...
			console.log(
				"No server credentials configured; running in sideloader-only mode.",
			);
		} catch (error) {
	...
	private async backgroundSync(): Promise<void> {
		try {
			...
			console.log(
				"[GameService] Background sync complete, notifying renderer.",
			);
		...
		try {
			...
			if (activeMirror) {
				...
				if (!configFilePath || !remoteName) {
					console.warn(
						"Failed to get mirror config file path, falling back to public endpoint",
					);
					...
					try {
						...
						const rcloneProcess = execa(rclonePath, rcloneArgs, {
							...
							env: mirrorApiKey
								? { RCLONE_HEADER: `X-API-Key: ${mirrorApiKey}` }
								: undefined,
						});
						...
						console.error(
							"Failed to use mirror config file:",
			...
			console.log(
				"[GameService] public endpoint: api key present:",
				!!publicApiKey,
			);
			...
			const rcloneProcess = execa(rclonePath, publicArgs, {
				stdio: ["ignore", "pipe", "pipe"],
				env: publicApiKey
					? { RCLONE_HEADER: `X-API-Key: ${publicApiKey}` }
					: undefined,
			});
			...
			if (result.exitCode !== 0) {
				throw new Error(
					`Rclone failed with exit code ${result.exitCode}: ${result.stderr}`,
				);
			}
			...
			console.error(
				"Error downloading meta archive:",
				redactKey(String(error)),
			);
			throw error;
		}
	...
	private async resolveGameListPath(): Promise<string | null> {
		...
		try {
			...
			console.error(
				"Error reading data path while resolving game list file:",
				error,
			);
		}
		return null;
	...
	private async loadGameList(): Promise<void> {
		try {
			...
			if (!resolvedPath) {
				console.error(
					"Game list file not found (looking for *amelist.txt in",
					this.dataPath,
					")",
				);
				return;
		...
		try {
			if (existsSync(this.customBlacklistPath)) {
				...
				try {
					...
					console.log(
						`Loaded ${this.customBlacklistGames.length} games from custom blacklist`,
					);
				} catch (parseError) {
	...
	private async saveCustomBlacklistGames(): Promise<void> {
		try {
			...
			console.log(
				`Saved ${this.customBlacklistGames.length} games to custom blacklist`,
			);
		} catch (error) {
	...
	private parseGameList(data: string): void {
		...
		for (let i = 1; i < lines.length; i++) {
			...
			try {
				...
				const gameName =
					gameNameIndex >= 0 ? parts[gameNameIndex].trim() : "Unknown";
				const packageName =
					packageNameIndex >= 0 ? parts[packageNameIndex].trim() : "";
				const versionCode =
					versionCodeIndex >= 0 ? parts[versionCodeIndex].trim() : "";
				const size = sizeIndex >= 0 ? `${parts[sizeIndex].trim()} MB` : "";
				const lastUpdated =
					lastUpdatedIndex >= 0 ? parts[lastUpdatedIndex].trim() : "";
				const releaseName =
					releaseNameIndex >= 0 ? parts[releaseNameIndex].trim() : "";
				const downloads =
					downloadsIndex >= 0 ? parts[downloadsIndex].trim() : "";
				...
				const thumbnailExists = packageName
					? thumbnailSet.has(thumbnailFile)
					: false;

				// Generate note path based on release name
				const notePath = releaseName
					? join(this.metaPath, "notes", `${releaseName}.txt`)
					: "";
	...
	async setServerConfig(config: {
		...
		if (config?.baseUri && config?.password) {
			...
			this.vrpConfig = null;
			console.log(
				"[GameService] Server config cleared at runtime (sideloader mode).",
			);
		}
	}
	...
	async getNote(releaseName: string): Promise<string> {
		...
		const userNote = await this.readCustomNote(
			this.customNotesPath,
			releaseName,
		);
		if (userNote) return userNote;

		const remoteNote = await this.readCustomNote(
			this.remoteNotesCachePath,
			releaseName,
		);
	...
	private async readCustomNote(
		...
		try {
			...
			console.warn(
				`[GameService] Failed to read custom notes from ${filePath}:`,
				err,
			);
		}
		return null;
	...
	async removeFromBlacklist(packageName: string): Promise<boolean> {
		...
		const index = this.customBlacklistGames.findIndex(
			(entry) => entry.packageName === packageName,
		);
		if (index === -1) {
			return false;
	...
	isGameBlacklisted(packageName: string, version?: number): boolean {
		...
		const entry = this.customBlacklistGames.find(
			(entry) => entry.packageName === packageName,
		);
		if (!entry) {
			return false;
