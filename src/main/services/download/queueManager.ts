		...
		try {
			if (existsSync(this.queuePath)) {
				...
				if (this.queue.length !== loadedQueue.length) {
					console.log(
						"Saving cleaned download queue after removing items with missing paths.",
					);
					await this.saveQueue(); // Save immediately after cleaning
				}

				console.log(
					`Loaded ${this.queue.length} download items from queue file.`,
				);
			} else {
	...
	public updateItem(
		...
		if (item) {
			...
			if (updates.extractProgress !== undefined) {
				item.extractProgress = Math.max(
					0,
					Math.min(100, updates.extractProgress),
				);
			} else if (
				updates.status &&
